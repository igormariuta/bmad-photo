...
### Post-completion fix-up round 1 (2026-07-08, user request — folder upload, plus a real regression it exposed)

- **Folder selection** — user asked to be able to pick a whole folder (or, one "Add folder" click at a time, several folders) instead of hand-selecting every photo inside it. A native `<input type="file" webkitdirectory multiple>` was added alongside the existing file input — a second "Add folder" button (`variant="outline"`, next to "Add photos") triggers it. `webkitdirectory` isn't a recognized JSX prop (non-standard attribute, no `@types/react` typing), so it's set imperatively via a ref callback rather than cast to `any`. Folder picks hand back every file in the tree, including non-image junk (`.DS_Store`, `Thumbs.db`) the input's own `accept="image/*"` can't filter out for a directory selection (browsers ignore `accept` in that mode) — this exact gap was flagged and deferred from this story's own original code review ("non-image file filtering → Story 2.2"); resolved now with a new `isImageFile` helper in `ingestPhotos.ts` (Story 2.2's file — see that story's own Dev Agent Record), filtering the selection before the cap check.
- **Real regression caught and fixed same round**: the refactor to filter non-image files accidentally reordered `event.target.value = ""` (the input-reset line) to run *before* `Array.from(files)` instead of after — `event.target.files` is a live `FileList` tied to the input, so resetting `value` clears it out from under any variable still holding that reference. Both "Add photos" and the new "Add folder" silently ingested nothing as a result (no error, no photos — just nothing happening). This exact gotcha is already correctly handled in `AddMoreControl.tsx`'s own handler (Story 2.5) with an explicit comment about it; this file's ordering was simply wrong. Fixed by copying to a plain array before the reset, matching `AddMoreControl.tsx`'s existing correct order.
- turbo lint/build/test clean (113/113 tests, 3 new for `isImageFile`); live-verified via Playwright — a synthetic folder (2 photos at the top level, 1 nested one level down, plus a `.DS_Store`) ingested exactly 3 photos via "Add folder", and a plain multi-file "Add photos" selection also still works correctly post-fix.
...
- `apps/gallery/src/features/ingest/EmptyState.tsx` (new; patched post-review — `role="alert"` wrapper, removed redundant `font-bold`; post-completion round 1 — added a second "Add folder" button/input, filters non-image files via `isImageFile`, fixed the input-reset-order regression)
...
- 2026-07-08 — Post-completion fix-up round 1 (user request) — added folder selection (new "Add folder" button, `webkitdirectory` input), finally resolving the non-image-file-filtering gap deferred from this story's original review (new `isImageFile` helper in Story 2.2's `ingestPhotos.ts`). The refactor briefly introduced a real regression — an input-reset-ordering bug that made both "Add photos" and "Add folder" silently ingest nothing — caught and fixed in the same round. `turbo lint/build/test` clean (113/113 tests, 3 new); live-verified with a synthetic folder (nested subfolder + junk file) via Playwright.
